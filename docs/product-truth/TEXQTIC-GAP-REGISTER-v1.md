...
Three tenant-facing misleading-authority surfaces remain open after closure of `GAP-TRUTH-001`
and `GAP-TRUTH-002` under `TRUTH-CLEANUP-001`: `GAP-TRUTH-003`, `GAP-TRUTH-004`, and
`GAP-TRUTH-005`. These are now bounded together under `TENANT-TRUTH-CLEANUP-001` as one shared
tenant truth-cleanup unit only.
