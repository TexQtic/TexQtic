...
- The most important exchange gap is failure to complete the first two-sided buyer-supplier loop in the product UI.
...
- `GAP-EXCHANGE-002` — tenant-visible trade execution remains backend-only
...
These are the strongest candidates for execution planning because they extend already-real backend capability into materially usable product loops.
