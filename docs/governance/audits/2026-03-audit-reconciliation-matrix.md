...
| Merged classification | `NEW_IN_COPILOT` |
| Merged status | `✅ CLOSED — G-028 C1 ✅ CLOSED (aaf8748 · 2026-03-15 · GOVERNANCE-SYNC-PW5-G028-C1-CONTROL-PLANE-AI-INSIGHTS); G-028 C2 ✅ CLOSED (a6eac77 · VERIFIED_COMPLETE · GOVERNANCE-SYNC-PW5-G028-C2-CONTROL-PLANE-AI-TARGETED); G-028 C5 ✅ CLOSED (d7e6629 · VERIFIED_COMPLETE · GOVERNANCE-SYNC-PW5-G028-C5-CONTROL-PLANE-AI-FRONTEND) — AiGovernance.tsx wired to POST /api/control/ai/insights; frontend-only; Slice 3 + Slice 4 remain open` |
| Confidence | HIGH |
...
| Wave 5 — Design-Required | TECS-FBW-012 (edit access); TECS-FBW-ADMINRBAC; TECS-FBW-AIGOVERNANCE (backend Slice 1 ✅ CLOSED · PW5-G028-C1 · aaf8748 · 2026-03-15; backend Slice 2 ✅ CLOSED · PW5-G028-C2 · a6eac77 · VERIFIED_COMPLETE; Slice 5 ✅ CLOSED (d7e6629 · VERIFIED_COMPLETE · GOVERNANCE-SYNC-PW5-G028-C5-CONTROL-PLANE-AI-FRONTEND)); TECS-FBW-013 (B2B quote); TECS-FBW-AUTH-001 (✅ CLOSED · 2026-03-13 · implemented commit 476b3d3) | REQUIRES_BACKEND_DESIGN or DEFERRED |
...
*Updated: 2026-03-15 — GOVERNANCE-SYNC-PW5-G028-B2-WORKER-BOOTSTRAP: CLOSED / VERIFIED_COMPLETE — production vector worker bootstrap added (commit f38df3b); concrete JobRunner (vectorWorker.ts) wraps each queued job in withDbContext(prisma, ctx, tx => executeVectorIndexJob(tx, job)); worker started from production bootstrap (index.ts post-listen); closes G-028-B2-DELETE-ENQUEUE-BLOCKER critical execution-path gap; both queued upsert and delete jobs now executable in real runtime path; typecheck EXIT 0 · 31/31 tests PASS; G-028-B2-DELETE-ENQUEUE-BLOCKER: CLOSED; non-blocking variances recorded (shutdown lifecycle / actor UUID); GOVERNANCE-SYNC-PW5-G028-B2-WORKER-BOOTSTRAP*
...
| ✅ CLOSED / VERIFIED_COMPLETE (f38df3b · 2026-03-15) | PW5-G028-B2-WORKER-BOOTSTRAP | Production vector worker bootstrap; concrete JobRunner (vectorWorker.ts) wraps each queued job in withDbContext(prisma, ctx, tx => executeVectorIndexJob(tx, job)); worker started from production bootstrap (index.ts post-listen); closes G-028-B2-DELETE-ENQUEUE-BLOCKER critical execution-path gap; orgId from job payload (set at enqueue time from dbContext.orgId); tenant-safe DB context + RLS doctrine preserved; no schema/migration/route/API/frontend/event-domain/control-plane widening; file scope: vectorWorker.ts (new) + index.ts (modified); typecheck EXIT 0 · 31/31 tests PASS (A6 queue 18/18 + ingestion 13/13); verification PASS; non-blocking variances: (1) onClose hook unreachable via real SIGTERM path (.unref() mitigates, non-defect); (2) actor UUID 00000000-0000-0000-0000-000000000001 new service-principal pattern (warrants doctrine naming follow-up only, non-defect) |
