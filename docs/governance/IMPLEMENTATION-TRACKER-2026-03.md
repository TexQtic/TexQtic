...
| ID | Objective | Affected Files | Risk | Merged Status | Exit Criteria | Status |
|---|---|---|---|---|---|---|
| TECS-FBW-001 | Add authority mutation methods + confirm UI — FinanceOps | services/controlPlaneService.ts; components/ControlPlane/FinanceOps.tsx | HIGH — payout approvals completely dark; SUPER_ADMIN cannot act | VALIDATED | approvePayoutMutation(), rejectPayoutMutation() in controlPlaneService; confirm-before-submit modal in FinanceOps.tsx; POST calls succeed | ✅ CLOSED — GOVERNANCE-SYNC-108 · 2026-03-07 · services/controlPlaneService.ts + components/ControlPlane/FinanceOps.tsx · FinanceAuthorityBody {reason?}; approvePayoutDecision+rejectPayoutDecision via adminPostWithHeaders with Idempotency-Key; PendingAction state; UUID generated at click time; loadPayouts() useCallback re-fetch on success; 200 replay = success; dialogError on failure; dialog copy uses 'Record Approval/Rejection Decision' — no money movement implied; reason-only input (no notes — finance body is {reason?} only); adminApiClient.ts unchanged (adminPostWithHeaders already present); typecheck EXIT 0 · lint EXIT 0 · git diff --name-only: 2 files only · next: TECS-FBW-001 Disputes sub-unit |
...
| TECS-FBW-001 | Add authority mutation methods + confirm UI — DisputeCases | services/controlPlaneService.ts; components/ControlPlane/DisputeCases.tsx | HIGH — dispute resolution dark | VALIDATED | resolveDispute(), escalateDispute() wired; confirm UI present | ✅ CLOSED — GOVERNANCE-SYNC-109 · 2026-03-07 · services/controlPlaneService.ts + components/ControlPlane/DisputeCases.tsx · DisputeAuthorityBody {resolution?,notes?}; resolveDispute+escalateDispute via adminPostWithHeaders with Idempotency-Key; ActionType 'resolve'\|'escalate'; PendingAction state; UUID generated at click time; loadDisputes() useCallback re-fetch on success; 200 replay = success; dialogError on failure; resolve/escalate button pair additive below card footer border; suppressed for c.status==='RESOLVED'; dialog: Resolution (optional) + Notes (optional) textareas labeled with htmlFor; field names match backend Zod exactly (resolution not reason); no SUPER_ADMIN gating (any-admin route); adminApiClient.ts unchanged; openapi.control-plane.json unchanged (routes already present); ComplianceQueue.tsx+FinanceOps.tsx unchanged; typecheck EXIT 0 · lint EXIT 0 · git diff --name-only: 2 files only · 🏁 TECS-FBW-001 ALL 3 SUB-UNITS COMPLETE (Compliance GOVERNANCE-SYNC-107 · Finance GOVERNANCE-SYNC-108 · Disputes GOVERNANCE-SYNC-109) · next: Wave 3 sequencing confirmation — TECS-FBW-002 (G-017 Trades) |
