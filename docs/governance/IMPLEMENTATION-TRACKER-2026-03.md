...
**Dependency:** VER-001 must resolve before TECS-FBW-PROV-001 is included or excluded.

| ID | Objective | Affected Files | Risk | Merged Status | Exit Criteria | Status |
|---|---|---|---|---|---|---|
| TECS-FBW-011 | Fix `basePrice` → `price` — SHIP BLOCKER | App.tsx (3 render sites); services/catalogService.ts (CatalogItem interface) | CRITICAL — every catalog item shows `$undefined.00` across all shells | VALIDATED | All catalog price renders display numeric values; no `p.basePrice` references remain | ⏳ Pending |
| TECS-FBW-PROV-001 | Fix provisionTenant() field mismatch (if VER-001 FAIL) | services/controlPlaneService.ts | HIGH — tenant creation silently fails if fields wrong | VERIFY_REQUIRED → promote VALIDATED if VER-001 FAIL | provisionTenant() body matches Zod schema; successful tenant creation confirmed | ⏳ Pending (gated on VER-001) |
...
| TECS-FBW-020 (if VER-002 FAIL) | Fix INVITE_MEMBER shell routing for WL_ADMIN | App.tsx (INVITE_MEMBER state rendering) | MEDIUM — shell context mismatch | VERIFY_REQUIRED → promote if FAIL | INVITE_MEMBER state renders correctly in WL_ADMIN context | ⏳ Pending (gated on VER-002) |
