...
| PW5-W3 | Control-plane Settlement admin read panel | 🔴 BACKEND DESIGN GATE | Inspection complete. `server/src/routes/control/settlement.ts` contains only `POST /api/control/settlements/preview` + `POST /api/control/settlements` (mutation endpoints). No read endpoint exists. **Blocked pending backend route: `GET /api/control/settlements`** — do not begin frontend wiring until this route is designed and implemented. |
...
**PW5-W3 backend design** — define `GET /api/control/settlements` read route (Settlement Admin, currently a three-layer absence: no AdminView token · no component · no GET route).  
Reason: VER-003 ✅ COMPLETE · VER-004 ✅ COMPLETE · SPEC-SYNC ✅ COMPLETE (commit 88ba3e7) · OPENAPI-POLICY-DECISION-GOV ✅ COMPLETE — governance blocker removed. OPENAPI-AI-SCOPE-001 ✅ RESOLVED (policy decision: /api/ai/* retained in tenant OpenAPI scope). OPENAPI-IMPERSONATION-DOC-001 ✅ RESOLVED (policy decision: impersonation routes intentionally excluded from public OpenAPI). PW5-W3 🔴 BACKEND DESIGN GATE (no `GET /api/control/settlements`). Deferred items remain deferred: tenant logout (DEF-003), B2B avatar (DEF-004), B2C search (DEF-005), WL storefront completion (PW5-WL1..3), DPP backend audit.
