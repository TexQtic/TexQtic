...
**Governance decision required** — AI route scope classification + impersonation documentation policy (OPENAPI-AI-SCOPE-001 · OPENAPI-IMPERSONATION-DOC-001); **then PW5-W3 backend design** — define `GET /api/control/settlements` read route before any UI wiring begins.  
Reason: VER-003 ✅ COMPLETE · VER-004 ✅ COMPLETE · SPEC-SYNC ✅ COMPLETE (commit 88ba3e7, 2026-03-10) — deterministic OpenAPI drift remediated. Two policy-class residuals remain OPEN: (1) `/api/ai/*` scope classification — AI routes remain in tenant spec pending contract ownership decision (OPENAPI-AI-SCOPE-001); (2) impersonation documentation policy — `POST /api/control/impersonation/start` + `/stop` remain undocumented pending governance decision (OPENAPI-IMPERSONATION-DOC-001). PW5-W3 🔴 BACKEND DESIGN GATE (no `GET /api/control/settlements`). Deferred items remain deferred: tenant logout (DEF-003), B2B avatar (DEF-004), B2C search (DEF-005), WL storefront completion (PW5-WL1..3), DPP backend audit.
