...
| PW5-W3 | Control-plane Settlement admin read panel | 🔴 BACKEND DESIGN GATE | Inspection complete. `server/src/routes/control/settlement.ts` contains only `POST /api/control/settlements/preview` + `POST /api/control/settlements` (mutation-type endpoints). No `GET` list or read endpoint exists. Cannot wire a read panel. Backend must design and implement a `GET /api/control/settlements` list endpoint before frontend wiring can begin. |
