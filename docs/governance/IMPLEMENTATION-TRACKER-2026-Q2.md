...
| Align `WLOrdersPanel` + `EXPOrdersPanel` order status display with canonical lifecycle (remove `deriveStatus()` audit-log hack) | Yes | � **UNBLOCKED** (GOVERNANCE-SYNC-062): API now exposes `lifecycleState` + `lifecycleLogs`. Awaiting B6b UI implementation. | B6b — UI implementation pending |
...
| Frontend order detail — lifecycle state badge + transition history | Yes | ❌ Pending B6b | B6b — unblocked by B6a |
