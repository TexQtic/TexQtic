...
| Align `WLOrdersPanel` + `EXPOrdersPanel` order status display with canonical lifecycle (remove `deriveStatus()` audit-log hack) | Yes | 🔴 **STOP CONDITION** (GOVERNANCE-SYNC-060): `deriveStatus()` reads `order.lifecycle.*` from `GET /api/tenant/audit-logs`; B4 removed those audit writes; new orders regress to showing “Placed” for CONFIRMED/FULFILLED. Fix requires `GET /api/tenant/orders` to include `order_lifecycle_logs: { take: 1 }` and expose `lifecycleState: string \| null` per order. `server/src/routes/tenant.ts` NOT in B5 allowlist. Panels NOT modified. | B6 — API surface enrichment required first |
| Frontend order detail — lifecycle state badge + transition history | Yes | ❌ | B6 — deferred pending API surface change |
