...
| **G-026 TECS 6C1 — Backend resolver endpoint** | ⏳ Planned | server/src/routes/internal.ts (NEW); server/src/lib/resolverHmac.ts (NEW); migration for texqtic_service role (G-026-H gate); GET /api/internal/resolve-domain; HMAC auth; narrow tenants(id,slug) query; requires G-026-H unblocked first | — |
| **G-026 TECS 6C2 — Edge middleware + context injection** | ⏳ Planned | middleware.ts (NEW); server/src/middleware/tenantResolution.ts (NEW); Fastify onRequest hook; x-texqtic-* header validation; JWT+domain coexistence; requires 6C1 validated | — |
| **G-026 TECS 6C3 — Cache + invalidation webhook** | ⏳ Planned | middleware.ts (MODIFY — cache Map+TTL); /api/internal/cache-invalidate (NEW); domain CRUD emits invalidation; requires 6C2 validated | — |
| **G-026 TECS 6D — WL Domains management panel** | ⏳ Planned | components/WhiteLabelAdmin/Domains.tsx (NEW); domain CRUD API + client; parallel with 6C3 | — |
