...
export const TenantRegistry: React.FC<TenantRegistryProps> = ({ onSelectTenant, onImpersonate }) => {
  const tenants = Object.values(PLATFORM_TENANTS);

  const getStatusColor = (status: TenantStatus) => {
    switch(status) {
      case TenantStatus.ACTIVE: return 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20';
      case TenantStatus.SUSPENDED: return 'text-rose-400 bg-rose-400/10 border-rose-400/20';
      case TenantStatus.TRIAL: return 'text-amber-400 bg-amber-400/10 border-amber-400/20';
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button className="bg-slate-100 text-slate-900 px-4 py-2 rounded font-bold text-xs uppercase tracking-tight">Provision New Tenant</button>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-800">
            {tenants.map(tenant => (
              <tr key={tenant.id} className="hover:bg-slate-800/30 transition-colors group cursor-pointer" onClick={() => onSelectTenant(tenant)}>
                ...
                <td className="px-6 py-4">
                  <span className={`px-2 py-0.5 rounded border text-[10px] font-black uppercase ${getStatusColor(tenant.status)}`}>
                    {tenant.status}
                  </span>
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4">
                  <div className="flex gap-3" onClick={(e) => e.stopPropagation()}>
                    <button onClick={() => onSelectTenant(tenant)} title="Config" className="p-1 hover:text-white text-slate-500">⚙️</button>
                    <button onClick={() => onImpersonate(tenant)} title="Impersonate" className="p-1 hover:text-blue-500 text-slate-500">👤</button>
                  </div>
                </td>
              </tr>
            ))}
