...
CREATE POLICY _prisma_migrations_deny_all ON public._prisma_migrations FOR ALL USING (false);
...
FROM anon,
  authenticated;
REVOKE ALL ON public.feature_flags
FROM anon,
  authenticated;
...
CREATE POLICY admin_users_deny_all ON public.admin_users FOR ALL USING (false);
DROP POLICY IF EXISTS feature_flags_deny_all ON public.feature_flags;
CREATE POLICY feature_flags_deny_all ON public.feature_flags FOR ALL USING (false);
-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
CREATE POLICY tenant_domains_tenant_delete ON public.tenant_domains FOR DELETE USING (
...
CREATE POLICY tenant_branding_tenant_delete ON public.tenant_branding FOR DELETE USING (
  current_setting('app.is_admin', true) = 'true'
);
...
CREATE POLICY tenant_feature_overrides_tenant_delete ON public.tenant_feature_overrides FOR DELETE USING (
  current_setting('app.is_admin', true) = 'true'
);
...
CREATE POLICY ai_budgets_tenant_delete ON public.ai_budgets FOR DELETE USING (
  current_setting('app.is_admin', true) = 'true'
);
...
CREATE POLICY ai_usage_meters_tenant_delete ON public.ai_usage_meters FOR DELETE USING (
  current_setting('app.is_admin', true) = 'true'
);
...
CREATE POLICY impersonation_sessions_admin_select ON public.impersonation_sessions FOR
SELECT USING (
    current_setting('app.is_admin', true) = 'true'
  );
DROP POLICY IF EXISTS impersonation_sessions_admin_insert ON public.impersonation_sessions;
CREATE POLICY impersonation_sessions_admin_insert ON public.impersonation_sessions FOR
INSERT WITH CHECK (
    current_setting('app.is_admin', true) = 'true'
  );
DROP POLICY IF EXISTS impersonation_sessions_admin_update ON public.impersonation_sessions;
CREATE POLICY impersonation_sessions_admin_update ON public.impersonation_sessions FOR
UPDATE USING (
    current_setting('app.is_admin', true) = 'true'
  );
DROP POLICY IF EXISTS impersonation_sessions_admin_delete ON public.impersonation_sessions;
CREATE POLICY impersonation_sessions_admin_delete ON public.impersonation_sessions FOR DELETE USING (
  current_setting('app.is_admin', true) = 'true'
);
-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
CREATE POLICY audit_logs_no_update ON public.audit_logs FOR
UPDATE USING (false);
-- DELETE: Admin-only (for compliance/retention management)
CREATE POLICY audit_logs_admin_delete ON public.audit_logs FOR DELETE USING (
  current_setting('app.is_admin', true) = 'true'
);
-- Additional protection: REVOKE UPDATE from standard roles
REVOKE
UPDATE ON public.audit_logs
FROM anon,
  authenticated;
-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
-- COMMIT AND REPORT
