...
ADD CONSTRAINT document_embeddings_uq UNIQUE (
    org_id,
    source_type,
    source_id,
    chunk_index,
...
CREATE INDEX idx_doc_embed_org_source ON public.document_embeddings (org_id, source_type, source_id);
-- Content hash lookup: check existence before triggering embedding generation.
CREATE INDEX idx_doc_embed_content_hash ON public.document_embeddings (content_hash);
...
CREATE INDEX idx_doc_embed_hnsw ON public.document_embeddings USING hnsw (embedding vector_cosine_ops) WITH (m = 16, ef_construction = 64);
-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
CREATE POLICY document_embeddings_guard ON public.document_embeddings AS RESTRICTIVE FOR ALL TO texqtic_app USING (
...
CREATE POLICY document_embeddings_select_unified ON public.document_embeddings AS PERMISSIVE FOR
...
CREATE POLICY document_embeddings_insert_unified ON public.document_embeddings AS PERMISSIVE FOR
...
GRANT SELECT,
  INSERT ON public.document_embeddings TO texqtic_app;
RAISE NOTICE 'G-028: Granted SELECT+INSERT on document_embeddings to texqtic_app';
ELSE RAISE NOTICE 'G-028: Role texqtic_app not found — skipping grant (test environment)';
END IF;
IF EXISTS (
  SELECT 1
  FROM pg_roles
...
GRANT SELECT,
  INSERT ON public.document_embeddings TO texqtic_admin;
RAISE NOTICE 'G-028: Granted SELECT+INSERT on document_embeddings to texqtic_admin';
ELSE RAISE NOTICE 'G-028: Role texqtic_admin not found — skipping grant (test environment)';
END IF;
END $$;
-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
v_guard_qual text;
BEGIN -- 1. pgvector extension enabled
SELECT EXISTS (
    SELECT 1
    FROM pg_extension
    WHERE extname = 'vector'
  ) INTO v_vector_ext;
IF NOT v_vector_ext THEN RAISE EXCEPTION 'G-028 VERIFY FAIL: pgvector extension (vector) is not installed.';
END IF;
-- 2. Table exists
SELECT EXISTS (
    SELECT 1
...
IF NOT v_table_exists THEN RAISE EXCEPTION 'G-028 VERIFY FAIL: public.document_embeddings table not found.';
END IF;
...
IF NOT v_rls_on THEN RAISE EXCEPTION 'G-028 VERIFY FAIL: RLS (relrowsecurity) not enabled on document_embeddings.';
END IF;
IF NOT v_rls_forced THEN RAISE EXCEPTION 'G-028 VERIFY FAIL: FORCE RLS (relforcerowsecurity) not set on document_embeddings.';
END IF;
...
IF v_col_count < 11 THEN RAISE EXCEPTION 'G-028 VERIFY FAIL: expected 11 columns on document_embeddings, found %.',
v_col_count;
END IF;
...
IF v_guard_count <> 1 THEN RAISE EXCEPTION 'G-028 VERIFY FAIL: expected 1 RESTRICTIVE guard on document_embeddings, found %.',
v_guard_count;
END IF;
...
WHERE schemaname = 'public'
  AND tablename = 'document_embeddings'
  AND permissive = 'RESTRICTIVE';
IF v_guard_qual NOT LIKE '%require_org_context%' THEN RAISE EXCEPTION 'G-028 VERIFY FAIL: document_embeddings guard is missing require_org_context arm.';
END IF;
IF v_guard_qual NOT LIKE '%is_admin%' THEN RAISE EXCEPTION 'G-028 VERIFY FAIL: document_embeddings guard is missing is_admin arm.';
END IF;
...
WHERE schemaname = 'public'
  ...
  AND cmd = 'SELECT';
IF v_perm_select <> 1 THEN RAISE EXCEPTION 'G-028 VERIFY FAIL: expected 1 PERMISSIVE SELECT policy on document_embeddings, found %.',
v_perm_select;
END IF;
...
WHERE schemaname = 'public'
  ...
  AND permissive = 'PERMISSIVE'
  AND cmd = 'INSERT';
IF v_perm_insert <> 1 THEN RAISE EXCEPTION 'G-028 VERIFY FAIL: expected 1 PERMISSIVE INSERT policy on document_embeddings, found %.',
v_perm_insert;
END IF;
-- 9. HNSW index exists
SELECT EXISTS (
...
IF NOT v_hnsw_index THEN RAISE EXCEPTION 'G-028 VERIFY FAIL: HNSW index idx_doc_embed_hnsw not found.';
END IF;
...
IF NOT v_uq_constraint THEN RAISE EXCEPTION 'G-028 VERIFY FAIL: UNIQUE constraint document_embeddings_uq not found.';
END IF;
