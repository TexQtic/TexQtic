...
CREATE UNIQUE INDEX IF NOT EXISTS traceability_nodes_org_batch_unique ON public.traceability_nodes(org_id, batch_id);
-- Composite query index: org + type lookups (most common filter pattern).
CREATE INDEX IF NOT EXISTS traceability_nodes_org_type_idx ON public.traceability_nodes(org_id, node_type);
-- ============================================================================
-- §3: updated_at trigger for public.traceability_nodes
-- ============================================================================
CREATE OR REPLACE FUNCTION public.traceability_nodes_set_updated_at() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN NEW.updated_at = now();
RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS trg_traceability_nodes_set_updated_at ON public.traceability_nodes;
CREATE TRIGGER trg_traceability_nodes_set_updated_at BEFORE
UPDATE ON public.traceability_nodes FOR EACH ROW EXECUTE FUNCTION public.traceability_nodes_set_updated_at();
-- ============================================================================
...
CREATE POLICY traceability_nodes_guard ON public.traceability_nodes AS RESTRICTIVE FOR ALL TO texqtic_app USING (
  app.require_org_context()
  OR app.bypass_enabled()
  OR current_setting('app.is_admin', true) = 'true'
);
COMMENT ON POLICY traceability_nodes_guard ON public.traceability_nodes IS 'RESTRICTIVE fail-closed guard. Permits access only when: org context set (tenant), bypass active (test/seed), or admin realm (is_admin=true). Mirrors G-019 admin-inclusive guard pattern.';
-- Tenant SELECT: own rows only (+ bypass)
DROP POLICY IF EXISTS traceability_nodes_tenant_select ON public.traceability_nodes;
CREATE POLICY traceability_nodes_tenant_select ON public.traceability_nodes AS PERMISSIVE FOR
SELECT TO texqtic_app USING (
    (org_id = app.current_org_id())
    OR app.bypass_enabled()
  );
COMMENT ON POLICY traceability_nodes_tenant_select ON public.traceability_nodes IS 'PERMISSIVE tenant SELECT: own org rows only (org_id = current_org_id) or bypass. No cross-tenant reads.';
-- Tenant INSERT: own tenant only (+ bypass)
DROP POLICY IF EXISTS traceability_nodes_tenant_insert ON public.traceability_nodes;
CREATE POLICY traceability_nodes_tenant_insert ON public.traceability_nodes AS PERMISSIVE FOR
...
COMMENT ON POLICY traceability_nodes_tenant_insert ON public.traceability_nodes IS 'PERMISSIVE tenant INSERT: require org context + org_id matches current_org_id, or bypass. Prevents cross-tenant INSERT.';
-- Tenant UPDATE: own rows only (+ bypass)
DROP POLICY IF EXISTS traceability_nodes_tenant_update ON public.traceability_nodes;
CREATE POLICY traceability_nodes_tenant_update ON public.traceability_nodes AS PERMISSIVE FOR
...
COMMENT ON POLICY traceability_nodes_tenant_update ON public.traceability_nodes IS 'PERMISSIVE tenant UPDATE: own org rows only. Prevents cross-tenant UPDATE.';
...
CREATE POLICY traceability_nodes_admin_select ON public.traceability_nodes AS PERMISSIVE FOR
SELECT TO texqtic_app USING (current_setting('app.is_admin', true) = 'true');
COMMENT ON POLICY traceability_nodes_admin_select ON public.traceability_nodes IS 'PERMISSIVE admin SELECT: cross-tenant read when app.is_admin=true (control/admin realm only). No tenant boundary enforced — audit/ops use only.';
-- ============================================================================
-- §6: GRANT on traceability_nodes
-- ============================================================================
GRANT SELECT,
  INSERT,
  UPDATE ON public.traceability_nodes TO texqtic_app;
-- ============================================================================
...
CREATE UNIQUE INDEX IF NOT EXISTS traceability_edges_unique_no_transform ON public.traceability_edges(org_id, from_node_id, to_node_id, edge_type)
WHERE transformation_id IS NULL;
-- Partial unique: when transformation_id IS NOT NULL — fully qualified.
CREATE UNIQUE INDEX IF NOT EXISTS traceability_edges_unique_with_transform ON public.traceability_edges(
  org_id,
  from_node_id,
  to_node_id,
  edge_type,
...
CREATE INDEX IF NOT EXISTS traceability_edges_org_from_idx ON public.traceability_edges(org_id, from_node_id);
CREATE INDEX IF NOT EXISTS traceability_edges_org_to_idx ON public.traceability_edges(org_id, to_node_id);
-- ============================================================================
...
CREATE POLICY traceability_edges_guard ON public.traceability_edges AS RESTRICTIVE FOR ALL TO texqtic_app USING (
  app.require_org_context()
  OR app.bypass_enabled()
  OR current_setting('app.is_admin', true) = 'true'
);
COMMENT ON POLICY traceability_edges_guard ON public.traceability_edges IS 'RESTRICTIVE fail-closed guard. Mirrors traceability_nodes_guard pattern.';
-- Tenant SELECT: own rows only (+ bypass)
DROP POLICY IF EXISTS traceability_edges_tenant_select ON public.traceability_edges;
CREATE POLICY traceability_edges_tenant_select ON public.traceability_edges AS PERMISSIVE FOR
SELECT TO texqtic_app USING (
    (org_id = app.current_org_id())
    OR app.bypass_enabled()
  );
COMMENT ON POLICY traceability_edges_tenant_select ON public.traceability_edges IS 'PERMISSIVE tenant SELECT: own org edges only.';
-- Tenant INSERT: own tenant only (+ bypass)
DROP POLICY IF EXISTS traceability_edges_tenant_insert ON public.traceability_edges;
CREATE POLICY traceability_edges_tenant_insert ON public.traceability_edges AS PERMISSIVE FOR
INSERT TO texqtic_app WITH CHECK (
    (
      app.require_org_context()
      AND org_id = app.current_org_id()
...
COMMENT ON POLICY traceability_edges_tenant_insert ON public.traceability_edges IS 'PERMISSIVE tenant INSERT: own org only.';
-- Tenant UPDATE: own rows only (+ bypass)
DROP POLICY IF EXISTS traceability_edges_tenant_update ON public.traceability_edges;
CREATE POLICY traceability_edges_tenant_update ON public.traceability_edges AS PERMISSIVE FOR
UPDATE TO texqtic_app USING (
    (org_id = app.current_org_id())
    OR app.bypass_enabled()
  ) WITH CHECK (
...
COMMENT ON POLICY traceability_edges_tenant_update ON public.traceability_edges IS 'PERMISSIVE tenant UPDATE: own org edges only.';
-- Admin SELECT: cross-tenant read when is_admin context is active
DROP POLICY IF EXISTS traceability_edges_admin_select ON public.traceability_edges;
CREATE POLICY traceability_edges_admin_select ON public.traceability_edges AS PERMISSIVE FOR
SELECT TO texqtic_app USING (current_setting('app.is_admin', true) = 'true');
COMMENT ON POLICY traceability_edges_admin_select ON public.traceability_edges IS 'PERMISSIVE admin SELECT: cross-tenant read when app.is_admin=true.';
-- ============================================================================
-- §11: GRANT on traceability_edges
-- ============================================================================
GRANT SELECT,
  INSERT,
  UPDATE ON public.traceability_edges TO texqtic_app;
-- ============================================================================
-- §12: Verification block — fails migration on any violated invariant
-- ============================================================================
DO $$
DECLARE v_orgs_exists boolean;
v_nodes_rls boolean;
...
v_edges_t_upd int;
v_edges_a_sel int;
BEGIN -- 1) organizations table must exist (G-015 dependency)
SELECT EXISTS (
    SELECT 1
...
SELECT relrowsecurity,
  relforcerowsecurity INTO v_nodes_rls,
  v_nodes_force
FROM pg_class
...
SELECT relrowsecurity,
  relforcerowsecurity INTO v_edges_rls,
  v_edges_force
FROM pg_class
...
WHERE schemaname = 'public'
  AND tablename = 'traceability_nodes'
  AND policyname = 'traceability_nodes_guard';
IF v_nodes_guard = 0 THEN RAISE EXCEPTION USING MESSAGE = '[G-016 FAIL] traceability_nodes_guard policy not found';
END IF;
...
WHERE schemaname = 'public'
  AND tablename = 'traceability_nodes'
  AND policyname = 'traceability_nodes_guard';
IF NOT v_nodes_guard_restr THEN RAISE EXCEPTION USING MESSAGE = '[G-016 FAIL] traceability_nodes_guard is not RESTRICTIVE';
END IF;
...
WHERE schemaname = 'public'
  AND tablename = 'traceability_nodes'
  AND policyname = 'traceability_nodes_guard';
IF NOT v_nodes_guard_admin THEN RAISE EXCEPTION USING MESSAGE = '[G-016 FAIL] traceability_nodes_guard does not include is_admin predicate';
END IF;
-- 5) traceability_nodes: tenant policies exist
SELECT COUNT(*) INTO v_nodes_t_sel
FROM pg_policies
WHERE schemaname = 'public'
  AND tablename = 'traceability_nodes'
  AND policyname = 'traceability_nodes_tenant_select';
SELECT COUNT(*) INTO v_nodes_t_ins
FROM pg_policies
WHERE schemaname = 'public'
  AND tablename = 'traceability_nodes'
  AND policyname = 'traceability_nodes_tenant_insert';
SELECT COUNT(*) INTO v_nodes_t_upd
FROM pg_policies
WHERE schemaname = 'public'
  AND tablename = 'traceability_nodes'
...
IF v_nodes_t_sel = 0
OR v_nodes_t_ins = 0
OR v_nodes_t_upd = 0 THEN RAISE EXCEPTION USING MESSAGE = format(
...
SELECT COUNT(*) INTO v_nodes_a_sel
FROM pg_policies
WHERE schemaname = 'public'
  AND tablename = 'traceability_nodes'
  AND policyname = 'traceability_nodes_admin_select';
IF v_nodes_a_sel = 0 THEN RAISE EXCEPTION USING MESSAGE = '[G-016 FAIL] traceability_nodes_admin_select policy not found';
END IF;
-- 7) traceability_edges: guard policy exists, is RESTRICTIVE, includes is_admin
SELECT COUNT(*) INTO v_edges_guard
FROM pg_policies
WHERE schemaname = 'public'
  AND tablename = 'traceability_edges'
  AND policyname = 'traceability_edges_guard';
IF v_edges_guard = 0 THEN RAISE EXCEPTION USING MESSAGE = '[G-016 FAIL] traceability_edges_guard policy not found';
END IF;
...
WHERE schemaname = 'public'
  AND tablename = 'traceability_edges'
  AND policyname = 'traceability_edges_guard';
IF NOT v_edges_guard_restr THEN RAISE EXCEPTION USING MESSAGE = '[G-016 FAIL] traceability_edges_guard is not RESTRICTIVE';
END IF;
...
WHERE schemaname = 'public'
  AND tablename = 'traceability_edges'
  AND policyname = 'traceability_edges_guard';
IF NOT v_edges_guard_admin THEN RAISE EXCEPTION USING MESSAGE = '[G-016 FAIL] traceability_edges_guard does not include is_admin predicate';
END IF;
-- 8) traceability_edges: tenant policies exist
SELECT COUNT(*) INTO v_edges_t_sel
FROM pg_policies
WHERE schemaname = 'public'
  AND tablename = 'traceability_edges'
  AND policyname = 'traceability_edges_tenant_select';
SELECT COUNT(*) INTO v_edges_t_ins
FROM pg_policies
WHERE schemaname = 'public'
  AND tablename = 'traceability_edges'
  AND policyname = 'traceability_edges_tenant_insert';
SELECT COUNT(*) INTO v_edges_t_upd
FROM pg_policies
WHERE schemaname = 'public'
  AND tablename = 'traceability_edges'
...
IF v_edges_t_sel = 0
OR v_edges_t_ins = 0
OR v_edges_t_upd = 0 THEN RAISE EXCEPTION USING MESSAGE = format(
...
SELECT COUNT(*) INTO v_edges_a_sel
FROM pg_policies
WHERE schemaname = 'public'
  AND tablename = 'traceability_edges'
  AND policyname = 'traceability_edges_admin_select';
IF v_edges_a_sel = 0 THEN RAISE EXCEPTION USING MESSAGE = '[G-016 FAIL] traceability_edges_admin_select policy not found';
END IF;
RAISE NOTICE '[G-016] PASS -- nodes: ENABLE/FORCE RLS t, guard RESTRICTIVE+admin t, tenant_select=%, tenant_insert=%, tenant_update=%, admin_select=% | edges: ENABLE/FORCE RLS t, guard RESTRICTIVE+admin t, tenant_select=%, tenant_insert=%, tenant_update=%, admin_select=%',
v_nodes_t_sel,
v_nodes_t_ins,
v_nodes_t_upd,
v_nodes_a_sel,
