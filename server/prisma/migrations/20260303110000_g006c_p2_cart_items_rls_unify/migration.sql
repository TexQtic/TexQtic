...
-- ─────────────────────────────────────────────────────────────
CREATE POLICY cart_items_guard ON public.cart_items AS RESTRICTIVE FOR ALL TO texqtic_app USING (
...
IF NOT v_force_rls THEN RAISE EXCEPTION 'VERIFIER FAIL: cart_items — relforcerowsecurity is false';
END IF;
...
IF v_guard_count <> 1 THEN RAISE EXCEPTION 'VERIFIER FAIL: cart_items — expected 1 RESTRICTIVE guard, found %',
v_guard_count;
END IF;
...
WHERE tablename = 'cart_items'
  AND permissive = 'RESTRICTIVE';
IF v_guard_qual NOT LIKE '%is_admin%' THEN RAISE EXCEPTION 'VERIFIER FAIL: cart_items guard — missing is_admin arm in USING predicate';
END IF;
IF v_guard_qual NOT LIKE '%require_org_context%' THEN RAISE EXCEPTION 'VERIFIER FAIL: cart_items guard — missing require_org_context arm in USING predicate';
END IF;
...
WHERE tablename = 'cart_items'
  ...
  AND cmd = 'SELECT';
IF v_perm_select <> 1 THEN RAISE EXCEPTION 'VERIFIER FAIL: cart_items — expected 1 PERMISSIVE SELECT, found %',
v_perm_select;
END IF;
...
WHERE tablename = 'cart_items'
  AND permissive = 'PERMISSIVE'
  AND cmd = 'INSERT';
IF v_perm_insert <> 1 THEN RAISE EXCEPTION 'VERIFIER FAIL: cart_items — expected 1 PERMISSIVE INSERT, found %',
v_perm_insert;
END IF;
...
WHERE tablename = 'cart_items'
  AND permissive = 'PERMISSIVE'
  AND cmd = 'UPDATE';
IF v_perm_update <> 1 THEN RAISE EXCEPTION 'VERIFIER FAIL: cart_items — expected 1 PERMISSIVE UPDATE, found %',
v_perm_update;
END IF;
...
WHERE tablename = 'cart_items'
  AND permissive = 'PERMISSIVE'
  AND cmd = 'DELETE';
IF v_perm_delete <> 1 THEN RAISE EXCEPTION 'VERIFIER FAIL: cart_items — expected 1 PERMISSIVE DELETE, found %',
v_perm_delete;
END IF;
...
WHERE tablename = 'cart_items'
  AND permissive = 'PERMISSIVE'
  AND cmd = 'SELECT';
IF v_sel_qual NOT LIKE '%is_admin%' THEN RAISE EXCEPTION 'VERIFIER FAIL: cart_items SELECT — missing is_admin arm (bypass_enabled wrongly used?)';
END IF;
...
IF v_public_count <> 0 THEN RAISE EXCEPTION 'VERIFIER FAIL: cart_items — found % {public} policies, expected 0',
v_public_count;
END IF;
