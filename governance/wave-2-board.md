...
- [x] G-005 — Standardize middleware: ALL tenant routes use `[tenantAuthMiddleware, databaseContextMiddleware]` — VALIDATED `830c0c4` — 10 routes migrated (tenant.ts: cart×4, checkout, orders×2, branding; ai.ts: insights, negotiation-advice) · 2 exclusions: /activate (invite-manual), GET /me (non-tenant-scoped) · buildContextFromRequest removed from both files · typecheck EXIT 0, lint 68w/0e
- [ ] G-006 — Align admin bypass to new context model (control-plane realm check replaces `app.is_admin`) — **BLOCKED** — `texqtic_app` role lacks GRANT on `admin_users`; implementation `f196445` reverted `c9ef413`; awaiting design decision (DB grant / prisma-direct / scope redefine). Deferred follow-ons logged as G-006C (admin-cart-summaries.ts legacy isAdmin) and G-006D (auth.ts tenantId 2-arg legacy).
