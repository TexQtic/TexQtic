...
Notes: PROD-AUDIT-001 persistent — no WL published passport in QA.
...
Status: VERIFIED_COMPLETE
Commit: 70bcac7
Title: Tenant Passport Registry — GET /api/tenant/dpp/passports, registry UI, 20 unit tests, DPP-E2E-24/25/26
...
Refs: components/Tenant/DPPPassport.tsx · server/src/routes/tenant.ts ·
  server/src/__tests__/tecs-dpp-passport-registry.test.ts · tests/e2e/dpp-passport-network.spec.ts ·
...
Status: VERIFIED_COMPLETE_WITH_LIMITATIONS
Commit: b1f580a
Title: Tenant DPP UX Productization — entry ladder, value summary, DPP-E2E-21/22/23
...
Summary: Productized the tenant DPP Passport page to present as a trust-building product surface.
  ...
  dpp-network-subtitle, dpp-network-value-summary, dpp-entry-ladder, dpp-entry-tier-{tier},
  dpp-manual-node-lookup), and secondary framing for node input ("Advanced: Load by...").
  isProductized flag derived from absence of title prop; white-label callers (App.tsx passing title)
  are completely unaffected.
  DPP-E2E-21/22/23 added as Group 9 source analysis tests (readFileSync pattern).
...
## 021 — 2026-05-15 — TECS-DPP-PASSPORT-NETWORK-021 Playwright E2E Environment Remediation
...
- playwright.config.ts exports plain object (no @playwright/test import) — Playwright runner resolves it internally.
...
- Fix: regex changed to /\{onNavigateDppLabel \?[\s\S]{0,400}/ — requires leading { char, only present
...
### Implementation
- tests/e2e/dpp-passport-network.spec.ts: line 1127 — 1-line regex change only.
...
### Commit
[TEXQTIC] test(dpp): remediate Playwright E2E environment — DPP-E2E-41/42 VERIFIED, fix DPP-E2E-38 regex
