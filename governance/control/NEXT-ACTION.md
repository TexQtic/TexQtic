...
**Authority:** governance/control/TEXQTIC-OPENING-LAYER-GOVERNANCE-AUTHORITY-AND-POINTER-LAYER-2026-04-10.md · **Updated:** 2026-06-11 (GOV-LAYER0-JULY-LAUNCH-READINESS-POINTER-SYNC-01 COMPLETE. FTR-B2C-005B RUNTIME_VERIFIED 2026-06-11. July launch window July 20–30. P0 gate queue active. Prior: FTR-SL-001T deploy + activation verification: OWNER membership created, public posture regressed. Preflight PASS (branch main, HEAD 695c0501). Deployment confirmed: app.texqtic.com → texqtic-cl53i4zpj-tex-qtic.vercel.app (Ready, created 10:47:11 IST / 05:17:11Z). GET /api/health → 200. Baseline PASS: GET /api/public/supplier/shraddha-industries → 200, directory total=2, invite 709598c3 accepted_at=NULL, membership count=0. ACTIVATION SUBMITTED ONCE: POST /api/tenant/activate → HTTP 200. Transaction timeout fix (FTR-SL-001S) CONFIRMED WORKING. Invite accepted_at = 2026-06-06 05:31:09.229+00. OWNER membership created: id=9307c62c-9425-48c9-a9f5-eac1735779b5, role=OWNER, tenant_id=0ae549d7-b17b-4277-b9f6-f3e8c3a57e09, email=shraddhaind@gmail.com, created_at=2026-06-06 05:31:08.274+00. NEW BLOCKER: Org status changed to PENDING_VERIFICATION by activation (was VERIFICATION_APPROVED). Public projection excludes PENDING_VERIFICATION. POST-ACTIVATION: GET /api/public/supplier/shraddha-industries → 404. Directory total=1, shraddha-industries MISSING. OWNER/ADMIN membership IS ready (verified). FTR-B2C-005B eligibility: OWNER membership verified, but public profile regressed — FTR-B2C-005B must wait for public posture restoration. Recommended next unit: control plane action to advance org 0ae549d7 status from PENDING_VERIFICATION back to VERIFICATION_APPROVED (or equivalent posture fix) to restore public visibility. No source/schema/env/SQL/package/deployment changes made. No corrective SQL. No reinvite. No live inquiry. FTR-LEGAL-003, FAM-07, FTU-COMM-002D, FTR-OPS-001 holds unchanged. FTR-B2C-005B not opened/closed. HD-001 status → OWNER_MEMBERSHIP_READY_PUBLIC_POSTURE_REGRESSED_PENDING_VERIFICATION. Final enum: FTR_SL_001T_BLOCKED_PUBLIC_PROFILE_REGRESSED_AFTER_ACTIVATION. Prior: FTR-SL-001S activation transaction timeout hardening: IMPLEMENTED. Preflight PASS (branch main, HEAD 4cf052ab, clean working tree). Source fix: explicit `{ timeoutMs: 30000, maxWaitMs: 30000 }` added as 4th arg to `withDbContext` activation call at tenant.ts:7007. Transaction boundary inspection PASS: callback contains ONLY DB operations + synchronous logic — no email, no external HTTP, no fetch/axios, no SMTP. tx.tenant.update remains absent. TypeScript: 2 pre-existing unrelated errors only. Activation tests: 30/30 PASS. No schema/migration/RLS/SQL/env/package changes. Invite 709598c3 still valid (accepted_at=NULL, expires 2026-06-13). FTR-LEGAL-003, FAM-07, FTU-COMM-002D, FTR-OPS-001 holds unchanged. FTR-B2C-005B blocked until OWNER/ADMIN membership verified. HD-001 status → ACTIVATION_TX_TIMEOUT_HARDENED_PENDING_PRODUCTION_DEPLOY_AND_VERIFY. Final enum: FTR_SL_001S_ACTIVATION_TX_TIMEOUT_FIX_IMPLEMENTED. Prior: FTR-SL-001R activation deployment verified + new blocker isolated: Preflight PASS (branch main, HEAD bfeb0e4d, clean working tree). Deployment confirmed: app.texqtic.com → texqtic-n8n47ix53-tex-qtic.vercel.app (Ready, created 04:17:08Z). Source fix from FTR-SL-001Q IS live. FTR-SL-001Q root cause (tx.tenant.update) is absent from production code. Baseline: GET /api/public/supplier/shraddha-industries → 200; directory total=2, shraddha-industries present. Invite 709598c3 accepted_at=NULL (still valid, expires 2026-06-13). Membership count=0 before activation. ACTIVATION STILL FAILS: POST /api/tenant/activate → 500. NEW ROOT CAUSE (Vercel logs confirmed): Prisma P2028 'Transaction not found. Transaction ID is invalid, refers to an old closed transaction' at prisma.invite.update() in tenant.ts:7083. The withDbContext $transaction call at tenant.ts:7007 has NO timeout option set (defaults to Prisma 5s). Cold-start DB connection latency (other endpoints show 16-17s response times in this cold-start window) causes the transaction to exceed the 5s default timeout before reaching invite.update. Fix required: pass txOptions.timeoutMs (e.g. 30000) to the withDbContext call in the activation route, OR configure Prisma transaction timeout globally. File: server/src/routes/tenant.ts (activation route withDbContext call, ~line 7007). No source/schema/env/SQL changes made. No corrective SQL. No reinvite. Invite still valid. FTR-LEGAL-003, FAM-07, FTU-COMM-002D, FTR-OPS-001 holds unchanged. FTR-B2C-005B remains blocked. HD-001 status → ACTIVATION_P2028_TRANSACTION_TIMEOUT_SERVERLESS_COLD_START. Final enum: FTR_SL_001R_BLOCKED_ACTIVATION_STILL_FAILS. Prior: FTR-SL-001Q RLS-safe activation source fix: IMPLEMENTED. Preflight PASS (branch main, HEAD c0a86766, clean working tree). tx.tenant.update removed from POST /api/tenant/activate withDbContext block in server/src/routes/tenant.ts. No RLS/schema/migration/SQL/env/package/deployment changes. No corrective SQL. No reinvite. No production activate call. TypeScript typecheck: 2 pre-existing errors in gate-d7 test file (unrelated, confirmed pre-existing on stash HEAD before change — zero new errors introduced by this fix). Targeted activation test suite: 30/30 PASS (tenant-activate.integration.test.ts). Secret/token grep: clean. Activation no longer attempts UPDATE on tenants table. Response shape unchanged (tenant populated by resolveTenantSessionIdentity not updatedTenant). FTR-LEGAL-003, FAM-07, FTU-COMM-002D, FTR-OPS-001 holds unchanged. FTR-B2C-005B remains blocked (no OWNER/ADMIN membership yet). HD-001 status → ACTIVATION_SOURCE_FIX_IMPLEMENTED_PENDING_PRODUCTION_DEPLOY_AND_VERIFY. Final enum: FTR_SL_001Q_ACTIVATION_RLS_SAFE_FIX_IMPLEMENTED. Recommended next unit: FTR-SL-001R deploy activation fix and verify Shraddha membership. Prior: FTR-SL-001P activation failure root-cause isolated: Preflight PASS (branch main, HEAD ebfe0e67, clean working tree). Public state confirmed: GET /api/public/supplier/shraddha-industries → 200 (B2B_PUBLIC, PUBLICATION_ELIGIBLE, legalName=Shraddha Industries), directory total=2, shraddha-industries present. DB checks (read-only): invite 709598c3 accepted_at=NULL (TX rolled back). User shraddhaind@gmail.com EXISTS (id=e5fb65ec-22b4-4889-99a6-434856b0db60, email_verified=true, created 2026-06-05, no membership). ROOT CAUSE: tx.tenant.update inside withDbContext (POST /api/tenant/activate in server/src/routes/tenant.ts) called when tenantData.name is non-empty. The tenants_deny_all RLS policy (PERMISSIVE, FOR ALL, qual=false) blocks ALL UPDATE operations for texqtic_app on the tenants table — confirmed by live DB test: UPDATE 0 with exact withDbContext role+config (SET LOCAL ROLE texqtic_app; app.realm=admin; app.is_admin=true). Prisma update() with 0 rows throws P2025 (record not found after RLS filter) → catch block → 500 INTERNAL_ERROR → apiClient 5xx → frontend "Service temporarily unavailable. Try again.". Invite still valid (expires 2026-06-13). No source/schema/env/SQL changes made. No invite reissued. FTR-LEGAL-003, FAM-07, FTU-COMM-002D, FTR-OPS-001 holds unchanged. HD-001 status → ACTIVATION_FAILURE_ROOT_CAUSE_ISOLATED_SOURCE_FIX_REQUIRED. Final enum: FTR_SL_001P_ROOT_CAUSE_IDENTIFIED_REQUIRES_SOURCE_FIX. Prior: FTR-SL-001O human-approved status restore + publish + reinvite. Preflight PASS (branch main, HEAD b60ca903, clean working tree). GET /api/health → 200 (2026-06-06T02:34:51.757Z). SUPER_ADMIN auth probe GET /api/control/tenants → 200. DB PREFLIGHT PASS: organizations row id=0ae549d7-b17b-4277-b9f6-f3e8c3a57e09, slug=shraddha-industries, legal_name=Shraddha Industries, status=CLOSED — confirmed. tenants row same id, slug=shraddha-industries, status=CLOSED, public_eligibility_posture=PUBLICATION_ELIGIBLE — confirmed. DB RESTORE PASS (Paresh-approved one-time op): UPDATE organizations SET status=VERIFICATION_APPROVED WHERE ...AND status=CLOSED RETURNING 1 row; UPDATE tenants SET status=ACTIVE WHERE ...AND status=CLOSED RETURNING 1 row. publication_posture and public_eligibility_posture NOT changed by SQL (already B2B_PUBLIC/PUBLICATION_ELIGIBLE). PUBLISH API PASS: POST /api/control/tenants/0ae549d7-.../publish → 200 success=true, alreadyPublished=true, publicationPosture=B2B_PUBLIC, publicEligibilityPosture=PUBLICATION_ELIGIBLE (idempotent — postures were already correct). PUBLIC VISIBILITY RESTORED: GET /api/public/supplier/shraddha-industries → 200 (slug=shraddha-industries, publicationPosture=B2B_PUBLIC, eligibilityPosture=PUBLICATION_ELIGIBLE, legalName=Shraddha Industries). Directory: GET /api/public/b2b/suppliers → 200, total=2, shraddha-industries present. REINVITE ISSUED: POST .../first-owner/reinvite → 200, inviteId=709598c3-212e-41dc-b5f4-bf2735ea41a5, expiresAt=2026-06-13T02:40:54.333Z, emailDispatched=true. No invite token/tokenHash/URL printed or stored. No source/schema/env changes. FTR-LEGAL-003, FAM-07, FTU-COMM-002D, FTR-OPS-001 holds unchanged. Final enum: FTR_SL_001O_STATUS_RESTORED_PUBLIC_RESTORED_FRESH_REINVITE_ISSUED. Recommended next unit: FTR-SL-001P-FRESH-INVITE-ACCEPTANCE-AND-MEMBERSHIP-VERIFY-001. Prior: FTR-SL-001N FTR_SL_001N_BLOCKED_PUBLISH_TENANT_CLOSED_STATUS_REQUIRES_DB_RESTORE.)
...
active_delivery_unit_note: >
  ...
  Pre-publish baseline: GET /api/public/supplier/shraddha-industries → 404 NOT_FOUND (confirmed unpublished).
  Directory baseline: GET /api/public/b2b/suppliers → total:1 (lt-b2b-001 only; Shraddha absent).
  (1) POST /api/control/tenants/0ae549d7-b17b-4277-b9f6-f3e8c3a57e09/publish → 200
      publicationPosture:B2B_PUBLIC, publicEligibilityPosture:PUBLICATION_ELIGIBLE, alreadyPublished:false.
  Post-publish: GET /api/public/supplier/shraddha-industries → 200
      (legalName:Shraddha Industries, orgType:B2B, jurisdiction:IN-GJ,
       publicationPosture:B2B_PUBLIC, eligibilityPosture:PUBLICATION_ELIGIBLE, no PII in response).
  Directory after: GET /api/public/b2b/suppliers → total:2 (Shraddha first, lt-b2b-001 second).
  (2) POST /api/control/tenants/0ae549d7-b17b-4277-b9f6-f3e8c3a57e09/first-owner/reinvite
      body.email:shraddhaind@gmail.com → 200
      inviteId:a96b7fd4-94b7-452c-97ea-5e95f60e3a89, expiresAt:2026-06-11T13:13:26.615Z,
      emailDispatched:true. No raw token in response (governance-compliant).
  Pending: Shraddha accepts invite → OWNER membership → GATE-SL-01 fully MET → FTR-B2C-005B unblocked.
  Final enum: FTR_SL_001D_SHRADDHA_PUBLISH_REINVITE_ACTIVATED.
last_closed_unit: FTR-SL-001D-SHRADDHA-INDUSTRIES-SUPPLIER-PRODUCTION-ACTIVATION-001
last_closed_unit_status: COMPLETE (2026-06-04)
last_closed_unit_runtime_verdict: >
  FTR-SL-001D COMPLETE (2026-06-04). No code changes — production activation only.
  POST /publish → 200 (B2B_PUBLIC, PUBLICATION_ELIGIBLE, alreadyPublished:false).
  POST /reinvite → 200 (inviteId:a96b7fd4-94b7-452c-97ea-5e95f60e3a89, emailDispatched:true).
  GET /api/public/supplier/shraddha-industries → 200 (Shraddha publicly visible, no PII).
  GET /api/public/b2b/suppliers → total:2 (Shraddha in directory as first entry).
  Invite expires: 2026-06-11T13:13:26.615Z. Pending: invite acceptance by Shraddha.
  Commit: "gov: activate Shraddha supplier publish and reinvite gate".
  Final enum: FTR_SL_001D_SHRADDHA_PUBLISH_REINVITE_ACTIVATED.
last_closed_unit_commits: '"gov: activate Shraddha supplier publish and reinvite gate"'
last_closed_unit_closure_basis: >
  FTR-SL-001D: production API calls only (no source/schema/env changes).
  POST /publish + POST /reinvite executed as SUPER_ADMIN from authenticated browser session.
  Evidence: API response data in active_delivery_unit_note above.
  Governance tracker updates: NEXT-ACTION.md, FUTURE-TODO-REGISTER.md, BLIND-SPOT register.
  Next: FTR-B2C-005B runtime verification unblocked pending Paresh authorization to call the new APIs.
last_closed_unit_prior: FTR-SL-001C-CONTROL-PLANE-SUPPLIER-PUBLISH-AND-REINVITE-API-001
...
last_closed_unit_prior2_status: COMPLETE (2026-06-04)
last_closed_unit_prior3: FTR-OPS-001B-SENTRY-ERROR-MONITORING-IMPLEMENTATION-001
