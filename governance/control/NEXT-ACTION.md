...
**Authority:** governance/control/TEXQTIC-OPENING-LAYER-GOVERNANCE-AUTHORITY-AND-POINTER-LAYER-2026-04-10.md · **Updated:** 2026-06-06 (FTR-SL-001N human-assisted republish attempt. Preflight PASS (branch main, HEAD 45d91fd5, clean working tree). GET /api/health → 200. Regression still present: GET /api/public/supplier/shraddha-industries → 404 NOT_FOUND. SUPER_ADMIN session confirmed: auth probe GET /api/control/tenants → 200 (tokenPresent true; JWT from browser localStorage; no token value returned or stored). PUBLISH BLOCKED: POST /api/control/tenants/0ae549d7-b17b-4277-b9f6-f3e8c3a57e09/publish → 409 TENANT_PUBLISH_STATUS_CONFLICT ("Tenant cannot be published from status CLOSED"). NEW CRITICAL FINDING: Shraddha org status = CLOSED in production DB. Publish route requires ACTIVE or VERIFICATION_APPROVED; CLOSED fails Gate D of six-gate safety layer. No API route exists to restore CLOSED → ACTIVE/VERIFICATION_APPROVED (only a close route exists, no reopen route). Resolution requires Paresh-approved direct Supabase SQL to restore org + tenant status to VERIFICATION_APPROVED before publish can proceed. No reinvite attempted. No invite token used or stored. No source/schema/env changes. FTR-LEGAL-003, FAM-07, FTU-COMM-002D, FTR-OPS-001 holds unchanged. Final enum: FTR_SL_001N_BLOCKED_PUBLISH_TENANT_CLOSED_STATUS_REQUIRES_DB_RESTORE. Prior: FTR-SL-001M FTR_SL_001M_BLOCKED_PUBLIC_SUPPLIER_REGRESSION_CONFIRMED.)
