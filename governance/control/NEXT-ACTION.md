...
**Authority:** governance/control/TEXQTIC-OPENING-LAYER-GOVERNANCE-AUTHORITY-AND-POINTER-LAYER-2026-04-10.md · **Updated:** 2026-06-05 (FTR-SL-001L operational run executed. Preflight clean on main at 25df4f4e. Deployment probe PARTIAL_PASS: GET /api/health → 200 with timestamp; exact deployed SHA not exposed in available runtime payloads. Public profile check PASS: GET /api/public/supplier/shraddha-industries → 200 (slug shraddha-industries, B2B_PUBLIC, PUBLICATION_ELIGIBLE, no top-level email/contact/pricing fields). SUPER_ADMIN session/auth probe FAIL in shared browser context: GET /api/control/tenants?page=1&limit=1 → 401 UNAUTHORIZED (Invalid or expired admin token). Fresh reinvite call BLOCKED: POST /api/control/tenants/0ae549d7-b17b-4277-b9f6-f3e8c3a57e09/first-owner/reinvite body.email shraddhaind@gmail.com → 401 UNAUTHORIZED (Invalid or expired admin token). Invite acceptance not attempted because no fresh private invite could be issued in this run. Membership readiness verification blocked by same control-plane auth failure. Final enum: FTR_SL_001L_BLOCKED_REINVITE_API_FAILED. Prior: FTR-SL-001J operational blocker run.)
