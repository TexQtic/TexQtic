...
| **1H (Settlement/Dispute/Quality/Bond)** | TEXQTIC-NC-PHASE1-POOL-SETTLE-001 + prereq TEXQTIC-NC-PHASE1-POOL-SETTLE-SCHEMA-001 (IMPLEMENTED_AWAITING_PARESH_VERIFY) | Pool settlement split + lifecycle SETTLED; quality gate stub; dispute hook | ⏸ SCHEMA_PREREQ_DELIVERED — `NetworkSettlementSplit` migration + `nc.settlement_waterfall.enabled=false` seeded. Packet 20 service/routes NOT authorized until Paresh verifies prereq. See governance/TEXQTIC-NC-PHASE1-POOL-SETTLE-SCHEMA-001.md. | 1G complete; schema prereq delivered; `nc.settlement_waterfall.enabled=false` seeded |
...
| 20 | **TEXQTIC-NC-PHASE1-POOL-SETTLE-001** | A/CPP | Service+Route | Pool settlement split computation; `NetworkSettlementSplit` schema; state SETTLED | Packet 19 COMPLETE; prereq TEXQTIC-NC-PHASE1-POOL-SETTLE-SCHEMA-001 IMPLEMENTED_AWAITING_PARESH_VERIFY | Service + route (schema prereq delivered) | Service + route | ⏸ SCHEMA_PREREQ_DELIVERED — `network_settlement_splits` table + `nc.settlement_waterfall.enabled=false` seeded (commit pending). Service + routes NOT YET AUTHORIZED. Awaiting Paresh verification of TEXQTIC-NC-PHASE1-POOL-SETTLE-SCHEMA-001. |
