...
| **1H (Settlement/Dispute/Quality/Bond)** | TEXQTIC-NC-PHASE1-POOL-SETTLE-001 (repo-truth validated: BLOCKED_PREREQ_MISSING) | Pool settlement split + lifecycle SETTLED; quality gate stub; dispute hook | ⛔ BLOCKED_PREREQ_MISSING — `NetworkSettlementSplit` schema ABSENT; `nc.settlement_waterfall.enabled` ABSENT. Prereq packet required. See governance/TEXQTIC-NC-PHASE1-POOL-SETTLE-001.md §6. | 1G complete; `nc.settlement_waterfall.enabled` flag required; `NetworkSettlementSplit` migration required |
...
| 20 | **TEXQTIC-NC-PHASE1-POOL-SETTLE-001** | A/CPP | Service+Route | Pool settlement split computation; `NetworkSettlementSplit` schema; state SETTLED | Packet 19 COMPLETE; `nc.settlement_waterfall.enabled` flag | Service + schema + route | Service + schema + route | ⛔ BLOCKED_PREREQ_MISSING — `NetworkSettlementSplit` schema ABSENT (no table/migration/service/route); `nc.settlement_waterfall.enabled` flag ABSENT from all migrations. Prereq packet required: `TEXQTIC-NC-PHASE1-POOL-SETTLE-SCHEMA-001`. See governance/TEXQTIC-NC-PHASE1-POOL-SETTLE-001.md. |
