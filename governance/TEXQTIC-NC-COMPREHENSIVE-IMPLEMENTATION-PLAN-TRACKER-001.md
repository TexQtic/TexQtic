...
| **1G (NC Invoice)** | TEXQTIC-NC-PHASE1-NC-INVOICE-COMPLETE-001 (Packet 19) | listNetworkInvoicesForPool + 2 GET routes (list + detail); ncPoolFeatureGateMiddleware | ⚠️ IMPLEMENTED_AWAITING_PARESH_VERIFY — tsc EXIT 0, 19/19 unit PASS, 12 integration tests ADDED (DB-gated, NOT yet executed against live Supabase) | 1F complete |
...
| 19 | **TEXQTIC-NC-PHASE1-NC-INVOICE-COMPLETE-001** | A/CPP | Service+Route | Complete `networkInvoice.service.ts`; add NC invoice routes; integrate with Pool Order | Packet 18 COMPLETE | Service + route + tests | Service + route | IMPLEMENTED_AWAITING_PARESH_VERIFY |
