...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Runtime taxonomy visibility | `TS + WAVE + PLAN + VOCAB` | `PARTIALLY_IMPLEMENTED_INTERNAL_ONLY` | `PARTIALLY_IMPLEMENTED_INTERNAL_ONLY` | Control-plane tenant identity interpretation visibility | Authenticated production control-plane verification on tenant registry and tenant detail, plus registry-to-detail-to-registry smoke | Deeper runtime descriptor semantics remain after the next visible read-model slice | Base family or internal category, capability or overlay, and plan are visibly separated where runtime identity is inspected |
...
| Control-plane identity interpretation | `TS + PLAN + VOCAB + ADMIN` | `NOT_YET_OPENED` | `NOT_YET_OPENED` | Registry/detail read-model split for base family or internal category, capability or overlay, and plan | Authenticated production control-plane verification on registry and deep-dive, plus neighbor-path smoke between those views | None at the current repo-truth layer | Control-plane registry and detail stop presenting raw `tenant_category` as complete architecture truth |
