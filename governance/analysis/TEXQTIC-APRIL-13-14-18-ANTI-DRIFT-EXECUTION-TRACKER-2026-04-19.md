...
| Taxonomy / subcategory visibility | `TAX + PLAN + VOCAB + EXPOSURE` | `BLOCKED_BY_CONTRACT_OR_SCHEMA` | `BLOCKED_BY_CONTRACT_OR_SCHEMA` | `NONE_READY` until one runtime-exposed taxonomy read-model seam is explicitly selected on top of the now-live persisted carrier | After runtime/API read-model support exists, verify lawful control-plane and tenant surfaces render primary segment, secondary segments, and role axis | Persisted canonical taxonomy carrier is now live; missing runtime/API read-model exposure still blocks lawful visibility verification | Primary segment, secondary segments, and role-position taxonomy are visible where lawful and remain B2B-owned |
