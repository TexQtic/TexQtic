...
Last Updated: 2026-03-03 (GOVERNANCE-SYNC-062 — GAP-ORDER-LC-001-API-LIFECYCLE-001 (B6a): `GET /api/tenant/orders` + `GET /api/tenant/orders/:id` enriched with `lifecycleState: string|null` (latest `to_state`) and `lifecycleLogs: { fromState, toState, realm, createdAt }[]` (take:5, newest-first); `OLLSelectRow` type alias added to avoid implicit-any from `withDbContext` proxy; backward-compatible (existing fields unchanged); typecheck EXIT 0; lint EXIT 0; unblocks B6b UI panel `deriveStatus()` removal)
...
| ------------- | ------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ----------- | ---------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| G-006C (RLS)  | **Multiple permissive RLS policies per command per table** — Supabase Performance Advisor flagged policy sprawl across 11 tables; eliminates OR-policy explosion before G-016–G-023 domains are added | `server/prisma/migrations/20260223010000…20260223110000` (11 original files) + P2 fix migrations | 🟠 Med  | IN PROGRESS | 20260223010000 – 20260303110000    | Pending `prisma migrate deploy` per table; P2 fix migrations correct admin arm (bypass_enabled → is_admin='true'); verification SQL embedded in each migration |
...
|---|---|---|---|---|---|---|
| GAP-ORDER-LC-001 | ORDER lifecycle schema prerequisite (Future Wave): add ORDER to LifecycleState CHECK constraint; add order lifecycle log table + RLS; seed ORDER states; extend SM EntityType union | Backend / DB | 🔴 HIGH | ⏳ **IN PROGRESS** — B1 ✅ (GOVERNANCE-SYNC-056): schema foundation; B2/SEED-001 ✅ (GOVERNANCE-SYNC-057): 4 ORDER transitions seeded. **B3/SM-SERVICE-001 ✅** (GOVERNANCE-SYNC-058, 2026-03-03): `EntityType` union extended; SM enforces ORDER transitions; `order_lifecycle_logs` written on success; SYSTEM_AUTOMATION FULFILLED target unlocked. **B4/BACKEND-INTEGRATION-001 ✅** (GOVERNANCE-SYNC-059, 2026-03-03): checkout workaround replaced with `tx.order_lifecycle_logs.create()`; PATCH `/tenant/orders/:id/status` app-layer if/else replaced with `StateMachineService.transition()`; DB enum mapping preserved; typecheck ✅. Remaining: frontend (order detail lifecycle badge + history); orders.status enum extension (ALTER TYPE ADD VALUE CONFIRMED/FULFILLED) still deferred. | GAP-ORDER-TRANSITIONS-001 (interim app-layer) | B4 complete: tenant.ts backend route integration done. Remaining: frontend only. |
