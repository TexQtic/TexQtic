...
| G-026  | Custom domain routing / tenant resolution (white-label) — stub only | **IN PROGRESS (Discovery — GOVERNANCE-SYNC-088)** | L scope; edge runtime + Prisma constraints; tenant_domains UNIQUE domain + tenant_id FK confirmed; no host-header routing exists; 3 insertion options documented; 7 sub-gaps registered (G-026-A thru G-026-G); design TECS next |
