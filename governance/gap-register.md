...
|---|---|---|---|---|---|---|
| GAP-ORDER-LC-001 | ORDER lifecycle schema prerequisite (Future Wave): add ORDER to LifecycleState CHECK constraint; add order lifecycle log table + RLS; seed ORDER states; extend SM EntityType union | Backend / DB | 🔴 HIGH | ⏳ **IN PROGRESS** — TECS B1 ✅ (GOVERNANCE-SYNC-056, 2026-03-03): `order_lifecycle_logs` created + RLS applied + CHECK constraints extended + 4 ORDER states seeded. **TECS B2/SEED-001 ✅** (GOVERNANCE-SYNC-057, 2026-03-03): 4 ORDER allowed_transitions seeded via `seed_state_machine.ts` (PAYMENT_PENDING→CONFIRMED, CONFIRMED→FULFILLED, CONFIRMED→CANCELLED, PAYMENT_PENDING→CANCELLED); VERIFIER PASS SEED_EXIT:0. Remaining: SM EntityType union extension (`ORDER` in TypeScript union), B3 SM wiring + route replacement, B4 frontend. orders.status enum extension deferred to B3 (ALTER TYPE ADD VALUE irreversible). | GAP-ORDER-TRANSITIONS-001 (interim app-layer) | B1 complete: schema foundation migration `20260315000005` applied. B2/SEED-001 complete: seed_state_machine.ts canonical (47 edges; idempotent upsert). Remaining B3-B4 unlocked. |
