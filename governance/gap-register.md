...
| G-026  | Custom domain routing / tenant resolution (white-label) | **Design Anchor ✅ — Implementation pending (GOVERNANCE-SYNC-089)** | L scope; D1=Hybrid Edge+Backend; D2=platform subdomains v1 (<slug>.texqtic.app); D3=HMAC resolver endpoint; D4=texqtic_service BYPASSRLS narrow; D5=60s Edge TTL cache; D6=internal signed contract; D7=x-texqtic-* headers; D8=fail-closed; G-026-A deferred v1.1; G-026-H blocking 6C1 (texqtic_service role); TECS 6C1/6C2/6C3/6D queued |
| G-026-H | `texqtic_service` DB role with BYPASSRLS SELECT on tenants(id,slug) not created — required for resolver endpoint | 🔴 OPEN — blocking TECS 6C1 deploy | SQL-only migration; no Prisma schema change; must pass governance review before 6C1 |
