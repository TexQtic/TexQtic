...
|--------|------------|----------------|------|--------|--------|-----------------|
| G-001 | **RLS policies check `app.tenant_id`; new routes set `app.org_id`** — policies do not fire | `server/prisma/rls.sql`; `server/src/lib/database-context.ts` | 🔴 High | NOT STARTED | — | Cross-tenant 0-row query must return 0 on all commerce tables |
