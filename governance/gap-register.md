...
|---|---|---|---|---|---|---|
| GAP-ORDER-LC-001 | ORDER lifecycle schema prerequisite (Future Wave): add ORDER to LifecycleState CHECK constraint; add order lifecycle log table + RLS; seed ORDER states; extend SM EntityType union | Backend / DB | 🔴 HIGH | ⏳ **IN PROGRESS** — TECS B1 ✅ (GOVERNANCE-SYNC-056, 2026-03-03): `order_lifecycle_logs` created + RLS applied + CHECK constraints extended + 4 ORDER states seeded; remaining: B2 SM EntityType extension (`ORDER` in TypeScript union), B3 SM wiring + route replacement, B4 frontend. orders.status enum extension deferred to B3 (ALTER TYPE ADD VALUE irreversible). | GAP-ORDER-TRANSITIONS-001 (interim app-layer) | TECS B1 complete: schema foundation migration `20260315000005_gap_order_lc_001_schema_foundation` applied. Remaining B2-B4 unlocked. Non-goals B1: no server/src changes, no orders table column changes, no enum alteration. |
