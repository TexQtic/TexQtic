...
|---|---|---|---|---|---|---|
| GAP-ORDER-LC-001 | ORDER lifecycle schema prerequisite (Future Wave): add ORDER to LifecycleState CHECK constraint; add order lifecycle log table + RLS; seed ORDER states; extend SM EntityType union | Backend / DB | 🔴 HIGH | ⏳ **IN PROGRESS** — B1 ✅ (GOVERNANCE-SYNC-056): schema foundation; B2/SEED-001 ✅ (GOVERNANCE-SYNC-057): 4 ORDER transitions seeded. **B3/SM-SERVICE-001 ✅** (GOVERNANCE-SYNC-058, 2026-03-03): `EntityType` union extended; SM enforces ORDER transitions; `order_lifecycle_logs` written on success; SYSTEM_AUTOMATION FULFILLED target unlocked; typecheck ✅. Remaining: B4 frontend (order detail lifecycle badge + history). orders.status enum extension (ALTER TYPE ADD VALUE CONFIRMED/FULFILLED) still deferred — not in B3 TECS scope. | GAP-ORDER-TRANSITIONS-001 (interim app-layer) | B3 complete: stateMachine.types.ts + stateMachine.service.ts + stateMachine.guardrails.ts updated. `prisma db pull` + `generate` run. Remaining: B4 frontend only. |
