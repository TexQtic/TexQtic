...
| FAM-06 | Auth and Session Management | **SELECTED AS FIRST FULL FAMILY CYCLE** by `TEXQTIC-FIRST-FAMILY-CYCLE-SELECTION-001` (2026-07-14). Selection only — status remains NOT_ASSESSED. Open family cycle after Layer 0 HOLD_FOR_AUTHORIZATION releases; run `FAM-06-AUTH-SESSION-OPENING-REPO-TRUTH-AUDIT-001`; audit auth routes, session, reused-user edge case. Family cycle may NOT open until Layer 0 HOLD_FOR_AUTHORIZATION releases. | Reused-existing-user: BOUNDED_DEFERRED_REMAINDER per Layer 0; must-haves §2 rows A-1 through A-8 not assessed |
