...
| **Priority** | P2 |
| **Status** | PARTIALLY_RESOLVED_WITH_PRIVATE_PRICING_DECISION — D11 advance (2026-06-04): FAM-13B-D11-PRICING-TIER-STRUCTURE-FINALIZATION-PR-08-CLOSURE-001. Items resolved/advanced: (1) first paid tier = STARTER (D1); (3) billing cycle = MONTHLY at launch; annual billing EXPLICITLY DEFERRED per D11 — no CA-approved annual equivalents in repo truth; annual can resume when CA confirms annual price equivalents + public-display authorization in a future unit. (4) GST display rule CA-CONFIRMED: SAC 998315, 18% GST, exclusive display (D2). (5) grace period = 3 days (D1). (6) non-payment action = downgrade to FREE, preserve data (D1). Item 2 advance (D11): provisional prices supplied (STARTER ₹2,499/mo, PROFESSIONAL ₹4,999/mo) — PRIVATE_PRICING_ONLY at launch; CA explicit public-display authorization absent from repo truth; founder explicitly elects private/manual/controlled launch pricing as the authorized launch path; public pricing page display BLOCKED until separate CA advisory confirms public-display authorization; operative prices remain valid for private/operator/manual contexts. PR-08 closed COMPLETE_WITH_PRIVATE_PRICING (2026-06-04). Item 7: feature entitlement scope per tier NOT yet supplied — still PARKED; required before FAM-11 self-serve billing implementation. D-011 full resolution requires: CA explicit public-display authorization (off-platform advisory + new governance unit) + feature entitlement scope defined by Paresh. |
