...
| BS-007 | Live public pages already indexed under app.texqtic.com — canonical domain not yet decided | All production-verified public pages (§2 of PUBLIC-PAGES-SEO-EXPANSION-REGISTER.md) are served from `app.texqtic.com`. The sitemap.xml canonical origin is hardcoded to `https://app.texqtic.com`. If Paresh's intended canonical domain is the apex (`texqtic.com`) or a custom branded domain, any organic impressions, backlinks, or GSC authority accumulated so far is being attributed to the wrong property. Once Google indexes at scale, migrating canonical domain requires careful redirect and sitemap surgery to preserve authority. | Domain authority built on the wrong property; SEO equity split; canonical migration required post-launch; potential ranking drop during migration. | sitemap.xml and all canonical tags use `app.texqtic.com`; D-005 (canonical domain strategy) is PARKED; live pages are production-indexed | Prioritize D-005 decision before any press release, paid backlinks, or GSC submission; do not build domain authority on a URL that may later change | P1 | OPEN |
