...
const MobileShellMenu: React.FC<MobileShellMenuProps> = ({
  ...
  const activeItemClassName = tone === 'dark'
          ...
          {items.map(item => {
            ...
            return (
              <button
                ...
                className={`w-full rounded-xl px-3 py-2.5 text-left text-sm font-medium transition ${item.active ? activeItemClassName : itemClassName}`}
              >
                {item.label}
              </button>
            );
...
export const B2BShell: React.FC<B2BShellProps> = ({
  ...
  const activeRouteKey = navigation.surface?.activeRouteKey ?? null;
  const mobileMenuItems: MobileShellMenuItem[] = isVerificationBlocked
    ? []
    : [
        { key: 'catalog', label: 'Catalog', active: activeRouteKey === 'catalog', onSelect: () => navigation.onNavigateRoute('catalog') },
        ...
        ...(hasShellRoute(navigation.surface, 'gst_verification') ? [{ key: 'gst_verification', label: 'GST Verification', active: activeRouteKey === 'gst_verification', onSelect: () => navigation.onNavigateRoute('gst_verification') }] : []),
        ...(hasShellRoute(navigation.surface, 'invoices') ? [{ key: 'invoices', label: 'Invoices', active: activeRouteKey === 'invoices', onSelect: () => navigation.onNavigateRoute('invoices') }] : []),
        ...
        ...(hasShellRoute(navigation.surface, 'traceability') ? [{ key: 'traceability', label: 'Traceability', active: activeRouteKey === 'traceability', onSelect: () => navigation.onNavigateRoute('traceability') }] : []),
        ...(hasShellRoute(navigation.surface, 'audit_logs') ? [{ key: 'audit_logs', label: 'Audit Log', active: activeRouteKey === 'audit_logs', onSelect: () => navigation.onNavigateRoute('audit_logs') }] : []),
        ...(hasShellRoute(navigation.surface, 'trades') ? [{ key: 'trades', label: 'Trades', active: activeRouteKey === 'trades', onSelect: () => navigation.onNavigateRoute('trades') }] : []),
        ...(hasShellRoute(navigation.surface, 'nc_pools') ? [{ key: 'nc_pools', label: 'NC Pools', active: activeRouteKey === 'nc_pools', onSelect: () => navigation.onNavigateRoute('nc_pools') }] : []),
        ...(hasShellRoute(navigation.surface, 'nc_pool_detail') ? [{ key: 'nc_pool_detail', label: 'Pool Detail', active: activeRouteKey === 'nc_pool_detail', onSelect: () => navigation.onNavigateRoute('nc_pool_detail') }] : []),
      ...
      <aside className="w-64 bg-slate-800 text-slate-300 hidden lg:flex flex-col p-6 sticky top-0 h-screen overflow-y-auto">
        ...
        {isVerificationBlocked ? (
          ...
          <nav className="flex-1 space-y-4">
            ...
            <button onClick={() => navigation.onNavigateRoute('catalog')} className={`w-full flex items-center gap-3 p-2 rounded text-left transition ${activeRouteKey === 'catalog' ? 'text-blue-300 bg-slate-700/60 font-medium' : 'text-slate-300 hover:text-white hover:bg-slate-700/50'}`}>📦 Catalog</button>
            ...
            {hasShellRoute(navigation.surface, 'escrow') && <button onClick={() => navigation.onNavigateRoute('escrow')} className={`w-full flex items-center gap-3 p-2 rounded text-left transition ${activeRouteKey === 'escrow' ? 'text-blue-300 bg-slate-700/60 font-medium' : 'text-slate-300 hover:text-white hover:bg-slate-700/50'}`}>� TradeTrust Ledger</button>}            {hasShellRoute(navigation.surface, 'gst_verification') && <button onClick={() => navigation.onNavigateRoute('gst_verification')} className={`w-full flex items-center gap-3 p-2 rounded text-left transition ${activeRouteKey === 'gst_verification' ? 'text-blue-300 bg-slate-700/60 font-medium' : 'text-slate-300 hover:text-white hover:bg-slate-700/50'}`}>🔐 GST Verification</button>}
            {hasShellRoute(navigation.surface, 'invoices') && <button onClick={() => navigation.onNavigateRoute('invoices')} className={`w-full flex items-center gap-3 p-2 rounded text-left transition ${activeRouteKey === 'invoices' ? 'text-blue-300 bg-slate-700/60 font-medium' : 'text-slate-300 hover:text-white hover:bg-slate-700/50'}`}>📄 Invoices</button>}            {hasShellRoute(navigation.surface, 'escalations') && <button onClick={() => navigation.onNavigateRoute('escalations')} className={`w-full flex items-center gap-3 p-2 rounded text-left transition ${activeRouteKey === 'escalations' ? 'text-blue-300 bg-slate-700/60 font-medium' : 'text-slate-300 hover:text-white hover:bg-slate-700/50'}`}>🚨 Escalations</button>}
            ...
            {hasShellRoute(navigation.surface, 'traceability') && <button onClick={() => navigation.onNavigateRoute('traceability')} className={`w-full flex items-center gap-3 p-2 rounded text-left transition ${activeRouteKey === 'traceability' ? 'text-blue-300 bg-slate-700/60 font-medium' : 'text-slate-300 hover:text-white hover:bg-slate-700/50'}`}>🔗 Traceability</button>}
            {hasShellRoute(navigation.surface, 'audit_logs') && <button onClick={() => navigation.onNavigateRoute('audit_logs')} className={`w-full flex items-center gap-3 p-2 rounded text-left transition ${activeRouteKey === 'audit_logs' ? 'text-blue-300 bg-slate-700/60 font-medium' : 'text-slate-300 hover:text-white hover:bg-slate-700/50'}`}>📋 Audit Log</button>}
            {hasShellRoute(navigation.surface, 'trades') && <button onClick={() => navigation.onNavigateRoute('trades')} className={`w-full flex items-center gap-3 p-2 rounded text-left transition ${activeRouteKey === 'trades' ? 'text-blue-300 bg-slate-700/60 font-medium' : 'text-slate-300 hover:text-white hover:bg-slate-700/50'}`}>🤝 Trades</button>}
            
            <div className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-4 pt-4 border-t border-slate-700">Network Commerce</div>
            {hasShellRoute(navigation.surface, 'nc_pools') && <button onClick={() => navigation.onNavigateRoute('nc_pools')} className={`w-full flex items-center gap-3 p-2 rounded text-left transition ${activeRouteKey === 'nc_pools' ? 'text-blue-300 bg-slate-700/60 font-medium' : 'text-slate-300 hover:text-white hover:bg-slate-700/50'}`}>💼 NC Pools</button>}
            {hasShellRoute(navigation.surface, 'nc_pool_detail') && <button onClick={() => navigation.onNavigateRoute('nc_pool_detail')} className={`w-full flex items-center gap-3 p-2 rounded text-left transition ${activeRouteKey === 'nc_pool_detail' ? 'text-blue-300 bg-slate-700/60 font-medium' : 'text-slate-300 hover:text-white hover:bg-slate-700/50'}`}>📋 Pool Detail</button>}
            {hasShellRoute(navigation.surface, 'nc_pool_demand_lines') && <button onClick={() => navigation.onNavigateRoute('nc_pool_demand_lines')} className={`w-full flex items-center gap-3 p-2 rounded text-left transition ${activeRouteKey === 'nc_pool_demand_lines' ? 'text-blue-300 bg-slate-700/60 font-medium' : 'text-slate-300 hover:text-white hover:bg-slate-700/50'}`}>📊 Demand Lines</button>}
...
export const B2CShell: React.FC<ShellProps> = ({ tenant, children, navigation }) => {
  ...
  const mobileMenuItems: MobileShellMenuItem[] = [
    ...
    ...(showAuthenticatedAffordances && hasShellRoute(navigation.surface, 'gst_verification') ? [{ key: 'gst_verification', label: 'GST Verification', onSelect: () => navigation.onNavigateRoute('gst_verification') }] : []),
    ...(showAuthenticatedAffordances && hasShellRoute(navigation.surface, 'invoices') ? [{ key: 'invoices', label: 'Invoices', onSelect: () => navigation.onNavigateRoute('invoices') }] : []),
      ...
      <div className="max-w-7xl mx-auto p-4 flex flex-wrap md:flex-nowrap items-center gap-4">
        ...
        {showAuthenticatedAffordances && (
          <div className="hidden md:flex gap-6 items-center">
            ...
            {hasShellRoute(navigation.surface, 'gst_verification') && <button onClick={() => navigation.onNavigateRoute('gst_verification')} className="text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-indigo-600 transition">GST Verification</button>}
            {hasShellRoute(navigation.surface, 'invoices') && <button onClick={() => navigation.onNavigateRoute('invoices')} className="text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-indigo-600 transition">Invoices</button>}
...
export const WhiteLabelShell: React.FC<ShellProps> = ({ tenant, children, navigation }) => {
  const mobileMenuItems: MobileShellMenuItem[] = [
    ...
    ...(hasShellRoute(navigation.surface, 'gst_verification') ? [{ key: 'gst_verification', label: 'GST Verification', onSelect: () => navigation.onNavigateRoute('gst_verification') }] : []),
    ...(hasShellRoute(navigation.surface, 'invoices') ? [{ key: 'invoices', label: 'Invoices', onSelect: () => navigation.onNavigateRoute('invoices') }] : []),
    ...
    <div className="min-h-screen font-sans" style={{ backgroundColor: '#ffffff' }}>
       ...
       <nav className="border-b border-slate-100 sticky top-0 bg-white/90 backdrop-blur-sm z-10">
          ...
          <div className="hidden md:flex justify-center gap-12 py-6 text-[10px] font-bold uppercase tracking-[0.2em]">
            ...
            {hasShellRoute(navigation.surface, 'escrow') && <button onClick={() => navigation.onNavigateRoute('escrow')} className="hover:opacity-40 transition-opacity">TradeTrust Ledger</button>}
            {hasShellRoute(navigation.surface, 'gst_verification') && <button onClick={() => navigation.onNavigateRoute('gst_verification')} className="hover:opacity-40 transition-opacity">GST Verification</button>}
            {hasShellRoute(navigation.surface, 'invoices') && <button onClick={() => navigation.onNavigateRoute('invoices')} className="hover:opacity-40 transition-opacity">Invoices</button>}
